...
async def complete_setup():
    ...
    print("🚀 Starting Complete MongoDB Atlas Setup...")
    print("=" * 60)
    ...
    try:
        ...
        employees = await create_employees(db, users)
        await create_availability(db, employees)
        schedules = await create_schedules(db, users[0], employees)
        await create_requests(db, employees)
...
async def create_employees(db, users):
    """Create employee profiles and additional employee users"""
    ...
    result = await db.employees.insert_one(demo_employee)
...
async def create_availability(db, employees):
...
async def create_schedules(db, admin_user, employees):
    ...
    for week_offset in [0, 1, 2]:
        ...
        total_hours = await create_shifts(db, schedule_id, week_start, employees)
...
async def create_shifts(db, schedule_id, week_start, employees):
...
async def create_requests(db, employees):
